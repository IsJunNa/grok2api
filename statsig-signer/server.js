...
async function loadCandidateTokens() {
  ...
  // Go 版账号 token 是加密存储的，签名机读不到 accounts.db。
  // x-statsig-id 是匿名反爬票据，不绑定账号身份；无 token 时用空会话启动即可。
...
async function launch(token) {
  ...
  const tok = (token || "").startsWith("sso=") ? token.slice(4) : (token || "").trim();
  if (tok) {
    ...
    log("launching without SSO cookies (anonymous statsig session)");
  }
